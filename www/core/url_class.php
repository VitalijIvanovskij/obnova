...
class URL {
	...
	public static function get($action, $controller = "", $data = array()/*массив get параметов*/, $amp = true, $address = "", $handler = true) {
		if ($amp) $amp = "&amp;";
		else $amp = "&";
		...
		if ($handler) return self::postHandler($address, $uri);
	...
	public static function deleteGET($url, $name, $amp = true) {
		$url = str_replace("&amp;", "&", $url);
		list($url_part, $qs_part) = array_pad(explode("?", $url), 2, ""); //разбивает url на 2 части, заносит части в массив, присваивает 1часть в переменную $url_part, 2часть в $qs_part
		parse_str($qs_part, $qs_vars);
		unset($qs_vars[$name]);
		if (count($qs_vars) != 0) {
	...
	private static function postHandler($address = "", $uri) {
		$uri = UseSEF::replaceSEF($address, $uri);
		return $uri;
