...
abstract class Controller extends AbstractController {
	...
	protected $meta_desc;		
	protected $meta_key;
	protected $mail = null;		//отправкаа писем
	protected $url_active;		//текущий url
	protected $section_id = 0;	//активный раздел
	protected $cat_id = 0;		//активная категория
	...
	final protected function render($str) {
		...
		$params["new_add"] = $this->getNewAdd();
		$params["top"] = $this->getTopMenu();
		$params["left"] = $this->getLeftMenu();
	...
	protected function getHead() {
		...
		$head->css = array("/styles/styles.css", "/styles/prettify.css");
		$head->js = array("/js/jquery-1.12.3.min.js", "/js/functions.js", "/js/validator.js", "/js/sstu_script.js");
		return $head;
	}
	...
	protected function getLeftMenu() {
		$items = LeftMenuDB::getLeftMenu();
		$leftmenu = new LeftMenu();
		$leftmenu->uri = $this->url_active;
	...
	protected function getNewAdd(){
		$items = NewAddDB::getNewAdd();
		$newadd = new NewAdd();
		$newadd->uri = $this->url_active;
		$newadd->items = $items;
		return $newadd;
	}
