...
class MainController extends Controller {
	...
	public function actionIndex() {
		$this->title = "Объявления Новопсков";
		$this->meta_desc = "Все объявления Новопскова и Новопсковского района собраны на одном сайте";
		$this->meta_key = "объявления, объявления новопсков, новопсковские объявления, объявления новопсковский район";
		
		$articles = ArticleDB::getAllShow(Config::COUNT_ADDS_ON_PAGE, $this->getOffset(Config::COUNT_ADDS_ON_PAGE), true);
		$pagination = $this->getPagination(ArticleDB::getCount(), Config::COUNT_ADDS_ON_PAGE, "/");
		$content = new Content();
		$content->articles = $articles;
		$content->pagination = $pagination;
		$this->render($this->renderData(array("content" => $content), "index"));
	}
	...
	public function actionSection() {
		...
		$blog = new Blog();
		$articles = ArticleDB::getAllOnPageAndSectionID($this->request->id, Config::COUNT_ADDS_ON_PAGE);
		...
		foreach ($more_articles as $id => $article) {
			...
			if ($i == Config::COUNT_ADDS_ON_PAGE) break;
		}
	...
	public function actionCategory() {
		...
		$articles = ArticleDB::getAllOnCatID($this->request->id, Config::COUNT_ADDS_ON_PAGE);
